...
turns the camera into a sub-half-second **live view in your browser**
...
4. Upload the platform with curl (self-signed HTTPS cert is expected) — **replace the
   placeholders first** (`PUT_YOUR_CAMERA_USER`/`PUT_YOUR_CAMERA_PASS` = your camera's REST
   login, `PUT_YOUR_CAMERA_IP` = your camera's address):
   ```
   curl.exe -ku PUT_YOUR_CAMERA_USER:PUT_YOUR_CAMERA_PASS -X PUT -H "Content-Type: application/xml" --data-binary "@streaming.xml" \
     "https://PUT_YOUR_CAMERA_IP/control/api/v1/livestreams/customPlatforms/Pyxis_SRT.xml"
   ```
...
**Set `CAMERA_USER` / `CAMERA_PASS` to your own camera's REST credentials — replace the
placeholders (`PUT_YOUR_CAMERA_USER`, `PUT_YOUR_CAMERA_PASS`, `PUT_YOUR_CAMERA_IP`) below with
your actual values.** The PYXIS REST API has **no documented factory-default password** (it
needs the web media manager enabled in Blackmagic Camera Setup, and authentication is whatever
the owner has configured — so use the login YOU set on your camera, not anything written here).
...
| `CAMERA_USER` | *(required — set it)* | Camera REST user |
| `CAMERA_PASS` | *(required — set it)* | Camera REST password |
...
Same-PC viewing needs no firewall changes. The first time you run the app, Windows
Firewall pops a security alert for each of `node.exe`, `ffmpeg.exe` and `mediamtx.exe` —
**accept ("Allow access") each prompt** and LAN viewers are covered automatically, no
per-port rules needed. You'll get the same prompts for the camera's SRT ingest on `9000`.

If the prompts were declined (or firewall rules were removed), LAN viewers then need these
inbound rules in Windows Firewall:

| Port | Why a viewer needs it |
|---|---|
| `9090` | page, REST API, WS relay |
| `8889` | mediaMTX WHEP handshake |
| `8189` | WebRTC media (UDP) |

`9000` (SRT) is only ever used by the *camera* connecting to this PC, never by browser
viewers.
...
- **Firewall:** if you accepted the first-run Windows prompts, nothing to do. Otherwise
  whitelist `8889`/`8189`/`9090` inbound — see *Ports used*.
